...
function createObjects() {
  ...
  var lightMapSquare = 100;
  light.position.set(3.2, 100, 0.5);
  light.target.position.set(0, 0, 0);
  light.shadow.camera.top = lightMapSquare;
  light.shadow.camera.bottom = - lightMapSquare;
  light.shadow.camera.left = - lightMapSquare;
  light.shadow.camera.right = lightMapSquare;
  light.shadow.camera.near = 1;
  light.shadow.camera.far = lightMapSquare + 14;
  ...
  /*
  const box = new THREE.Mesh(new THREE.BoxGeometry(5, 20, 10), material2);
  ...
  */
  ...
  /*glbloader.load("./models/fish.glb",(model)=>{
    const fish=model.scene.children[0];
    fish.position.y=20;   
    fish.scale.multiplyScalar(6); 
    fish.rotation.z=0.6;
    fish.material=material2;
    scene.add(fish);
    physics.createObj(fish,"mesh","obj",null,2);
    window.F=fish;
  });*/
...
function createDominoGrid({
  ...
  const startX = position.x - (cols / 2) * spacing;
  const startZ = position.z - (rows / 2) * spacing;
  const startY = position.y + dominoHeight / 2;
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      ...
      box.position.set(startX + j * spacing, startY, startZ + i * spacing);
...
function createDominoSpiralSimple({
  turns = 5,
  spacing = 14,           // Distância linear entre dominós
  minRadius = 14,
  direction = 1,          // 1 = horário, -1 = anti-horário
  startAngle = 0,         //Rotação da espiral em radianos
  position = { x: 0, y: 0, z: 0 },
  material = material2,
}) {
  const dominoWidth = 3;
  ...
  while (angle < totalAngle) {
    ...
    // Rotaciona a peça olhando para fora da curva
