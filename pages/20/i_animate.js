...
class ANIMATE {
    ...
    animateObjects(delta, timer) {
        ...
        var tvpower = getOByName('tv_power');
        var channel_light = getOByName('tv_led');
        ...
        if (typeof (getOByName('atari_switch1')) != _UN) {//if one exist all exist
            ...
            for (var i = 1; i < 7; i++) {
                ...
                var upP = { x: 0.095, y: 0, z: -1.8 };
                var dpP = { x: -0.55, y: -0.1, z: -2.2 };
                if (i != 1) {
                    dpP = { x: 0.1, y: 0, z: 0 };
                    upP = { x: 0.9, y: 0.1, z: 0.1 };
                }
                if (currswitch.push == 1) {
                    switch (currswitch.state) {
                        case 0: //down to up (+x)
                            // terminou descida
                            if (currswitch.position.x <= dpP.x) {
                                currswitch.state = 1;
                                currswitch.push = 0;
                                //action ON
                                if (i == 1) {
                                ...
                                if (i == 4)
                                    CTRL(jt.ConsoleControls.DIFFICULTY1, true);
                                if (i == 5)
                                    CTRL(jt.ConsoleControls.SELECT, false);
                                if (i == 6)
                                    CTRL(jt.ConsoleControls.RESET, false);
                            } else {
                                currswitch.position.x +=
                                    (dpP.x - currswitch.position.x) *
                                    (0.018 * switchspeed);
                                currswitch.position.y +=
                                    (dpP.y - currswitch.position.y) *
                                    (0.018 * switchspeed);
                                currswitch.position.z +=
                                    (dpP.z - currswitch.position.z) *
                                    (0.018 * switchspeed);
                                if (Math.abs(currswitch.position.x - dpP.x) < 0.1) {
                                    currswitch.position.set(dpP.x, dpP.y, dpP.z);
                        ...
                        case 1: //up to down (-x)
                            // terminou subida
                            if (currswitch.position.x >= upP.x) {
                                currswitch.state = 0;
                                if (currswitch.model == 0) {
                                    currswitch.push = 0; //switch button togle
                                    if (i == 1) {
                                ...
                                if (i == 5) //no togle back to top
                                    CTRL(jt.ConsoleControls.SELECT, true);
                                if (i == 6) //no togle back to top
                                    CTRL(jt.ConsoleControls.RESET, true);
                            } else {
                                currswitch.position.x +=
                                    (upP.x - currswitch.position.x) *
                                    (0.018 * switchspeed);
                                currswitch.position.y +=
                                    (upP.y - currswitch.position.y) *
                                    (0.018 * switchspeed);
                                currswitch.position.z +=
                                    (upP.z - currswitch.position.z) *
                                    (0.018 * switchspeed);

                                if (Math.abs(currswitch.position.x - upP.x) < 0.1) {
                                    currswitch.position.set(upP.x, upP.y, upP.z);
                                }
                            }
                            break;
                    }
        ...
        if (getOByName('room_wall1') != null && control != null && 1 == 2) { //DISABLED tranformar em RAYCAST
    ...
    animateAtari(delta,timer){
        ...
        if (materials[usefullChannels.atari.material].map == null && ataricanvas != null) {
            ...
            materials[usefullChannels.atari.material].map.colorSpace = THREE.SRGBColorSpace;
            materials[usefullChannels.atari.material].map.premultiplyAlpha = true;
            materials[usefullChannels.atari.material].map.generateMipmaps = false;
            materials[usefullChannels.atari.material].map.minFilter = THREE.NearestFilter;
            materials[usefullChannels.atari.material].map.magFilter = THREE.NearestFilter;
            materials[usefullChannels.atari.material].transparent = true;
            materials[usefullChannels.atari.material].alphaTest = 0.01;
            materials[usefullChannels.atari.material].needsUpdate = true
