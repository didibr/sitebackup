...
window.HOMESITE="https://didisoftwares.ddns.net/";
...
dracoLoader.setDecoderPath(HOMESITE+'draco/');
...
async function createMaterial() {
    ...
    */
    ...
    materials[100] = new THREE.MeshPhongMaterial({ color: 0xde2e03 }); //orange
...
async function init() {
    ...
    renderer = new THREE.WebGLRenderer({
        ...
        powerPreference: "low-power", //"low-power", //"high-performance", "low-power" or "default"
    ...
    irender.postprocessEnabled=false;
    ...
    if(isWebGL2Supported()==true){   
        if(irender.postprocessEnabled)       
        filters.N8AO.enabled=true; //true
    }
    window.GAME = { materials, objects, audios, transfCtrlShow, rayintersects,raycaster, filters,irender };
...
function animate() {
    ...
    if (delta && delta != null && iscene.creating == false && control && typeof (control != _UN)) {
        ...
        render();
        ianimate.animateAtari(test, timer);
    }
    ...
    if (code == 'Space') {        
        if (getOByName('joy_btn')) {            
            ...
            if (typeof (joyaudio) != _UN) {                
                getOByName('joy_btn').position.y = -0.20;                
    ...
    for (var e = 0; e < rayintersects.length; e++) {
        ...
        if (intercept.parent && intercept.parent.name) {
            if (intercept.parent.name.startsWith('cart_single')) { //cartrights 
                if(getOByName('stove_box'))//HAVE STOVE               
        ...
        if (intercept && intercept.name && typeof (intercept.name) == 'string') {
            ...
            if ( intercept.name == 'tv_channel_btn') { //change tv channel                
                 var tv_channel = getOByName('tv_channel_box');
                if (typeof (tv_channel.lastChannel) == _UN)tv_channel.lastChannel = 0;
                tv_channel.lastChannel += 1;
                tv_channel.channelChanged = 1;
                if (tv_channel.lastChannel > 12) tv_channel.lastChannel = 0;
                // console.log(intercept.parent.parent.lastChannel);
            }

            if (intercept.name == 'tv_power') { //TV power                
