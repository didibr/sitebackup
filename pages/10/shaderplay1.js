  ...
  function create(isEditor = false) {
    ...
    function resizeBuffersToMatchCanvas() {
      for (let i = 0; i < state.buffers.length; i++) {
        if (!state.buffers[i]) continue;

        if (
          state.buffers[i].width !== measuretCanvas().width ||
          state.buffers[i].height !== measuretCanvas().height
        ) {
          const oldProgram = state.buffers[i].program;

          state.buffers[i].destroy();
          state.buffers[i] = createFBO(measuretCanvas().width, measuretCanvas().height);

          state.buffers[i].program = oldProgram;
        }
      }
    }
    ...
    function applyUniform(program, name, value) {
      ...
      if (typeof value === 'number') {
        ...
        //detect big array (uniform array)
        if (value.length > 4) {
    ...
    function bindEvents() {
      ...
      function updateMousePosition(e) {
        ...
        // screen coords -> shader coords
        state.mouse.x =
          ((e.clientX - rect.left) / rect.width)
          * measuretCanvas().width;

        // invertido igual ShaderToy
        state.mouse.y =
      ...
      // eventos no canvas
      state.canvas.addEventListener("mousemove", mm);
      state.canvas.addEventListener("mousedown", md);
      ...
      return () => {
        ...
        state.canvas.removeEventListener("mousemove", mm);
        state.canvas.removeEventListener("mousedown", md);
    ...
    function render(time) {
      ...
      for (let i = 0; i < state.MAX_VIDEOS; i++) {
        ...
        if (video.readyState >= 2) {
          ...
          if (loc && gl.getParameter(gl.CURRENT_PROGRAM) === program) {
    ...
    function continueInit() {
      ...
      if (state.alreadyInitied) { resizeCanvas(); sampleExec(state.entryExample) }
    ...
    function setShader(vertexSource, fragmentSource, onUpdate = (t) => { }) {
      ...
      //state.errorDiv = state.root.getElementById('errordiv');
      ...
      if (state.isEditorMode) {
        ...
        if (state.codevertex) { state.codevertex.setValue(vertexSource) } else {
          vertArea.value = vertexSource;
          state.codevertex = CodeMirror.fromTextArea(vertArea, { lineNumbers: true, lineWrapping: true, mode: 'x-shader/x-vertex', theme: 'material-darker', gutters: ["CodeMirror-linenumbers", "error-gutter"], });
          state.codevertex.setSize("100%", "100%");
        }
        if (state.codefrag) { state.codefrag.setValue(fragmentSource) } else {
          fragArea.value = fragmentSource;
          state.codefrag = CodeMirror.fromTextArea(fragArea, { lineNumbers: true, lineWrapping: true, mode: 'x-shader/x-fragment', theme: 'material-darker', gutters: ["CodeMirror-linenumbers", "error-gutter"] });
          state.codefrag.setSize("100%", "100%");
        }
        clearErrors(state.codevertex);
        clearErrors(state.codefrag);
      } else {
        // 🔥 MODO SHADERTEXT (FAKE)
        if (!state.codevertex) {
      ...
      if (buffers) { for (let i = 0; i < buffers.length; i++) { if (buffers[i] && buffers[i].destroy) buffers[i].destroy(); } }
      buffers = [];
      texStatic = [];
      for (let i = 0; i < state.MAX_CHANNELS; i++) {
        const key = "Buffer" + i;
      ...
      if (!vs) { state.program = null; return }
      ...
      const compileAndSet = (source, target, line) => {
        ...
        if (fs) {
          ...
          if (newProgram) { target.program = newProgram } else { return }
        } else { fragNotCompiled = true; return }
      };
      for (let i = 0; i < state.MAX_CHANNELS; i++) { if (buffers[i]) buffers[i].program = {} }
      const mainProgram = {};
      for (let i = 0; i < state.MAX_CHANNELS; i++) {
        const key = "Buffer" + i;
        if (parsed[key] && parsed[key].code) {
          compileAndSet(parsed[key].code, buffers[i], parsed[key].line);
          if (fragNotCompiled) { state.program = null; return }
        }
      }
      if (parsed.Main.code) {
      ...
      if (state.errorDiv) {
        ...
        //console.log("%cSUCESS", "color: green;");
      }
      const program = state.program;
      state.sucessCodes = { vert: vertexSource, frag: fragmentSource };
      ...
      for (let i = 0; i < state.MAX_CHANNELS; i++) {
        state.iChannelLocations[i] = gl.getUniformLocation(program, 'iChannel' + i);
      }
      for (let key in state.customUniforms) {
    ...
    async function ShaderTextPath(element, vertexCode, fragmentCode, options = { glyph: true, letterSpacing: 0, frameSkip: 0, precision: "highp", alpha: false }) {
      ...
      window.removeEventListener("resize", resizeCanvas);
      window.removeEventListener("scroll", scrolPage);
      window.addEventListener("resize", resizeCanvas);
      window.addEventListener("scroll", scrolPage);

      if (state.root.fonts && state.root.fonts.addEventListener) {
        state.root.fonts.addEventListener("loadingdone", () => { resizeCanvas(); });
      }
    ...
    async function ShaderText(element, vertexCode, fragmentCode, options = { mode: "text", frameSkip: 0, precision: "highp", alpha: false }) {
      ...
      if (!el) return;
      if (options.glyph) {
        return ShaderTextPath(element, vertexCode, fragmentCode, options);
      }
      state.text = el;
      state.isEditorMode = false;
    ...
    function showEditor() {
      ...
      title.style.cssText = `display: flex;font-weight: bold;gap: 10px;font-family: sans-serif;background: linear-gradient(90deg, rgb(139, 139, 139), rgb(255, 255, 255), rgb(139, 139, 139)) 0% 0% / 200% 100% text;-webkit-text-fill-color: transparent;animation: 4s linear 0s infinite normal none running flow;  flex-direction: row; align-content: center; justify-content: center; align-items: center;`;
      const style = state.root.createElement("style");
      style.textContent = `@keyframes flow{from{background-position:0 0}to{background-position:200% 0}}a#logod svg{opacity:.7;transition:.2s}a#logod:hover svg{opacity:1;transform:rotate(12deg)}a#logod{display:flex;height:18px;font-size:x-small;text-decoration:none;flex-direction:row;align-items:stretch;flex-wrap:nowrap;gap:2px}`;
