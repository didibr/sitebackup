  ...
  function create(isEditor = false) {
    ...
    function createFBO_old(width, height) {
      ...
      function createTexture() {
        ...
        gl.texImage2D(
          glValue(gl, state.config.texture.target),
          glValue(gl, state.config.texture.level),
          glValue(gl, state.config.texture.internalFormat),
          width,
          height,
          glValue(gl, state.config.texture.border),
          glValue(gl, state.config.texture.format),
          glValue(gl, state.config.texture.type),
          null
        );
        ...
        gl.texParameteri(
          ...
          glValue(gl, state.config.texture.minFilter)
        );

        gl.texParameteri(
          glValue(gl, state.config.texture.target),
          gl.TEXTURE_MAG_FILTER,
          glValue(gl, state.config.texture.magFilter)
        );

        gl.texParameteri(
          glValue(gl, state.config.texture.target),
          gl.TEXTURE_WRAP_S,
          glValue(gl, state.config.texture.wrapS)
        );

        gl.texParameteri(
          glValue(gl, state.config.texture.target),
          gl.TEXTURE_WRAP_T,
          glValue(gl, state.config.texture.wrapT)
        );
    ...
    function createFBO(width, height) {
      ...
      const useFloat = state.config.texture.type === "FLOAT";
