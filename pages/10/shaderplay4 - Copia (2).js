  ...
  function create(isEditor = false) {
    ...
    var state = {
      ...
      config: {
        ...
        alpha: false,
        glyph: false,
        bufferFilter: "Nearest", // or Linear
        bufferTexture: "Float", // or UNSIGNED_BYTE
    ...
    function resizeBuffersToMatchCanvas_old() {
      const width = measuretCanvas().width;
      const height = measuretCanvas().height;
      const useFloat = state.config.bufferTexture === "Float";
    ...
    function resizeBuffersToMatchCanvas() {
      ...
      for (let i = 0; i < state.buffers.length; i++) {
        ...
        const newBuf = createFBO(width, height);
    ...
    function transformTextureCalls(src) {

      if (!state.config.pixelPerfect) return src;
      ...
      while (i < src.length) {
        ...
        if (src.startsWith("texture", i)) {
          ...
          let fullCall = src.slice(i, j);
          let inner = src.slice(startArgs, j - 1);
          ...
          if (args.length === 2) {
            ...
            // ✅ só aplica para buffers
            if (/^iChannel\d+$/.test(tex)) {

              let newCall;

              // 🔥 CASO SHADERTOY (coord / iResolution)
              if (uv.includes("/ iResolution.xy")) {

                let coord = uv.replace("/ iResolution.xy", "").trim();

                newCall = `texelFetch(${tex}, ivec2(${coord}), 0)`;

              } else {

                // ✅ fallback genérico
                newCall =
                  `texelFetch(${tex}, ivec2((${uv}) * iResolution.xy), 0)`;
              }
    ...
    function compileShader(gl, source, type, lineOffset) {
      ...
      if (state.config.pixelPerfect && state.webGlversion == 3) {
        source = transformTextureCalls(source);
      }
    ...
    function render(time) {
      ...
      for (let i = 0; i < state.MAX_AUDIOS; i++) {
        ...
        const gl = state.gl;
        const isWebGL2 = state.webGlversion == 3;
        ...
        gl.texSubImage2D(
          ...
          a.data.length,
          1,
          isWebGL2 ? gl.RED : gl.LUMINANCE,   // ✅ AQUI É A DIFERENÇA
          gl.UNSIGNED_BYTE,
          a.data
        );
...
if (state.config.alpha) {
    ...
    function render_new(time) {
      ...
      if (state.destroyed || !state.gl || !state.sucessCodes) return;
      ...
      // ✅ ================= SNAPSHOT (SHADERTOY CORE) =================
      const readTextures = {};
      for (let id of state.executionOrder) {
        if (buffers[id]) {
          readTextures[id] = buffers[id].read;
        }
      }
      ...
      for (let id of state.executionOrder) {
        ...
        const loc = cacheAttrib(buf.program, 'position');
        gl.enableVertexAttribArray(loc);
        gl.vertexAttribPointer(loc, 2, gl.FLOAT, false, 0, 0);

        // uniforms básicos
        gl.uniform3f(cacheUniform(buf.program, 'iResolution'), w, h, 1);
        gl.uniform1f(cacheUniform(buf.program, 'iTime'), time);
        gl.uniform1f(cacheUniform(buf.program, 'iTimeDelta'), deltaTime);
        gl.uniform1i(cacheUniform(buf.program, 'iFrame'), state.frameCount - 1);

        // ✅ CHANNELS (snapshot correto)
        for (let i = 0; i < state.MAX_CHANNELS; i++) {

          let tex = null;

          if (readTextures[i]) tex = readTextures[i];
          else if (texStatic[i]) tex = texStatic[i];
          ...
          if (tex) {
            ...
            const loc = cacheUniform(buf.program, 'iChannel' + i);
        ...
        for (let i = 0; i < state.MAX_AUDIOS; i++) {
          ...
          const unit = state.MAX_CHANNELS + i;

          gl.activeTexture(gl.TEXTURE0 + unit);
          gl.bindTexture(gl.TEXTURE_2D, tex);

          const loc = cacheUniform(buf.program, 'iAudio' + i);
        ...
        for (let i = 0; i < state.MAX_VIDEOS; i++) {
          ...
          const unit = state.MAX_CHANNELS + state.MAX_AUDIOS + i;

          gl.activeTexture(gl.TEXTURE0 + unit);
          gl.bindTexture(gl.TEXTURE_2D, tex);

          const loc = cacheUniform(buf.program, 'iVideo' + i);
          if (loc !== null) gl.uniform1i(loc, unit);
        }
      ...
      gl.vertexAttribPointer(
        state.positionAttributeLocation,
        2,
        gl.FLOAT,
        false,
        0,
      ...
      gl.uniform3f(state.iResolutionLocation, w, h, 1);
      gl.uniform1f(state.iTimeLocation, time);
      gl.uniform1f(state.iTimeDeltaLocation, deltaTime);
      ...
      for (let i = 0; i < state.MAX_AUDIOS; i++) {
        ...
        const loc = cacheUniform(program, 'iAudio' + i);
    ...
    /*
    if (
        /\btexelFetch\s*\(/.test(src) ||
        /\blayout\s*\(/.test(src) ||
        /\bflat\s+in\b/.test(src) ||
        /\bflat\s+out\b/.test(src)
    ) {
        return 3;
    }
        */
    ...
    function createFBO_old(width, height) {
      ...
      function createTexture() {
        ...
        gl.texImage2D(
          ...
          0,
          gl.RGBA,
          state.config.bufferTexture === "Float" ? gl.FLOAT : gl.UNSIGNED_BYTE,
          null
        );
        ...
        gl.texParameteri(
          ...
          state.config.bufferFilter === "Nearest" ? gl.NEAREST : gl.LINEAR
        );
    ...
    function createFBO(width, height) {
      ...
      function createTexture(initialData = null) {
        ...
        // ✅ filtro respeitando config
        const filter =
          state.config.bufferFilter === "Nearest"
            ? gl.NEAREST
            : gl.LINEAR;
    ...
    function setShader(vertexSource, fragmentSource, onUpdate = (t) => { }) {
      ...
      state.webGlversion = detectGLSLVersion(vertexSource, fragmentSource);
      ensureGLContext(state.webGlversion);
