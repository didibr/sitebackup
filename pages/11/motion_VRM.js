...
retargetAnimations(clip, animationModel, targetModel, rigMap, scaleHips = true) {
 ...
 async load(input, animations = null) {
  ...
  if (this.debug) {
    if (!this.baseClone) {
      this.baseClone = this.cloneSkinnedModelSafe(this.baseTPoseModel);
      this.baseClone.position.x = 150;
      this.scene.add(this.baseClone);
    }

    if (window.bmixer) {
      window.bmixer.stopAllAction();
      window.bmixer.uncacheRoot(this.baseClone);
    }

    window.bmixer = new THREE.AnimationMixer(this.baseClone);
  }
  ...
  const actions = parsedClips.reduce((acc, clip) => {
    ...
    if (window.bmixer && clip) {
      const baseAction = window.bmixer.clipAction(clip);
      if (baseAction) {
        window.X[clip.name] = baseAction;
        baseAction.clampWhenFinished = true;
        baseAction.zeroSlopeAtEnd = true;
      }
    }
...
async addAnimation2(model, input, finalName) {
  ...
  reloadAnimations(model) {
    ...
    if (oldMixer) {
      ...
      if(window.bmixer){
      window.bmixer.stopAllAction();
      window.bmixer.uncacheRoot(model);  
      }
    }
