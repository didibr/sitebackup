...
export class MixamoIntegration {
  ...
  async loadModel(url, config = null) {
    ...
    model.traverse(obj => {
      if (obj.isBone) {
        obj.name = obj.name.replace(/^mixamorig\d*/g, 'mixamorig');
  ...
  reloadModelAnimations(model) {
    ...
    const parsedClips = this.rawAnimationData.map((a) => {
      const clip = THREE.AnimationClip.parse(a);
      clip.name = this.#stripPrefix(clip.name);
      clip.tracks.forEach(track => {
        track.name = this.#stripPrefix(track.name);
      });
      this.#removeScaleTracks(clip);
      return clip;
    });
  
    // Retarget para o modelo específico
    this.#retargetClipsToModel(parsedClips, model);
    ...
    const actions = parsedClips.reduce((acc, clip) => {
      ...
      // ⚠️ limpa bindings caso algum tenha sido alterado por _setPartial
      if (action._originalState) {
        ...
        delete action._originalState;
      }
  
      acc[clip.name] = action;
      return acc;
    }, {});
  ...
  async _setPartial(model, boneNames, animationName, lerpTime = 0.3) {
  ...
  #stripPrefix = name => name.replace(/^Armature\|/, '').replace(/mixamorig\d*/g, 'mixamorig');
