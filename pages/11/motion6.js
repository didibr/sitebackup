...
const accurigRigMap = {
  ...
  "CC_Base_Waist": "",
  "CC_Base_Spine01": "mixamorigSpine",
  "CC_Base_Spine02": "mixamorigSpine2",
  "CC_Base_NeckTwist01": "",
  "CC_Base_NeckTwist02": "mixamorigNeck",
  "CC_Base_Head": "",
  "CC_Base_FacialBone": "",
  "CC_Base_JawRoot": "",
  "CC_Base_Tongue01": "",
...
renameBonesToMixamo(model, rigMap) {
  if (!rigMap) return;
  model.traverse((bone) => {
    if (bone.isBone && rigMap[bone.name]) {
      bone.name = rigMap[bone.name]; // ← Troca nome para padrão Mixamo
    }
  });
}

createTestClip() {
  const times = [0, 0.5, 1.0]; // Tempo em segundos

  // Levanta o braço (rotação no eixo X)
  const qStart = new THREE.Quaternion().identity();
  const qMid = new THREE.Quaternion().setFromEuler(new THREE.Euler(-Math.PI / 2, 0, 0)); // levanta
  const qEnd = new THREE.Quaternion().identity(); // volta ao normal
  ...
  // Move os quadris para frente e para trás
  const transValues = new Float32Array([
    0, 0, 0,
    0, 0.0, 0.5,
    0, 0, 0
  ]);
  ...
  return new THREE.AnimationClip("TestRetargetClip", -1, [armTrack, hipsTrack]);
}

createTestClip2() {
  const times = [0]; // Tempo em segundos
...
*/
  return new THREE.AnimationClip("hi", -1, [armTrack, /*hipsTrack*/]);
}
 ...
 async load(input, animations = null) {  
  ...
  this.rawAnimationData = [
    ...
    this.createTestClip().toJSON(),
    this.createTestClip2().toJSON(),
...
/*const rigMap =
    model.getObjectByName("mixamorigHips") ? mixamoRigMap
  //: model.getObjectByName("Bip01") || model.getObjectByName("bip_Hips") ? bipedRigMap
  : model.getObjectByName("CC_Base_Hips") || model.getObjectByName("CC_Base_Spine01") ? accurigRigMap
  : null;
  */

  model.userData.type="mixamo";
  if(model.getObjectByName("CC_Base_Hips") || model.getObjectByName("CC_Base_Spine01")){
    model.userData.type="accurig";
    this.renameBonesToMixamo(model, accurigRigMap);
  }
  ...
  const parsedClips = this.rawAnimationData.map((a) => {
    ...
    clip.tracks.forEach((track) => {
      ...
      clip = this.retargetAnimations(clip, animationModel, model,true,model.userData.type);
...
//const testClip = this.createTestClip();
//const json = THREE.AnimationClip.toJSON(testClip);
//console.log(JSON.stringify(json)); 
  ...
  reloadAnimations(model) {
    ...
    const _qCTemp = new THREE.Quaternion();

    const parsedClips = this.rawAnimationData.map((a) => {
      const clip = THREE.AnimationClip.parse(a);
      clip.name = clip.name.replace(/^Armature\|/, '');

      // Substitui os tracks corretamente
      clip.tracks = clip.tracks.map((track) => {
        ...
        if (
          ...
          const _qC = new THREE.Quaternion();
          const newValues = new Float32Array(track.times.length * 4);
          for (let i = 0; i < newValues.length; i += 4) {
            // Valor de rotação fixo para levantar o braço
            _qCTemp.fromArray(values, i);
            _qC.set(_qCTemp.z, _qCTemp.y, _qCTemp.x, _qCTemp.w);
          ...
          for (let i = 0; i < values.length; i += 3) {
            ...
            // Exemplo: trocar eixo Z por X
            newValues[i] = x;   // novo X
            newValues[i + 1] = -z;   // mantém Y
            newValues[i + 2] = y;   // novo Z
          }
 ...
 retargetAnimations(clip, animationModel, targetModel, scaleHips = true, modelType) {
  const tracks = [];
  ...
  const motionHips = animationModel.getObjectByName("mixamorigHips");
  ...
  for (const track of clip.tracks) {
    ...
    const targetBone = targetModel.getObjectByName(sourceName);
    ...
    if (track instanceof THREE.QuaternionKeyframeTrack) {
      ...
      for (let i = 0; i < values.length; i += 4) {
        ...
        if (modelType !== "accurig") {
          // Aplica retarget tradicional (Mixamo)
          _qC.premultiply(parentWorld).multiply(restInv);
        } else {
          // Corrige rotação para modelo AccuRIG
          const euler = new THREE.Euler().setFromQuaternion(_qC, "XYZ");
          const correctedEuler = new THREE.Euler(euler.z, euler.y, -euler.x, "XYZ");
          _qC.setFromEuler(correctedEuler);
        }
    ...
    // 📦 POSITION (VectorKeyframeTrack) — normalmente só para o "hips"
    else if (track instanceof THREE.VectorKeyframeTrack && sourceName === "hips") {
      const values = track.values;
      const newValues = new Float32Array(values.length);

      for (let i = 0; i < values.length; i += 3) {
        const x = values[i] * hipsPositionScale;
        const y = values[i + 1] * hipsPositionScale;
        const z = values[i + 2] * hipsPositionScale;

        if (modelType === "accurig") {
          // Corrige eixos: X, Y, Z → X, -Z, Y
          newValues[i + 0] = x;
          newValues[i + 1] = -z;
          newValues[i + 2] = y;
        } else {
  ...
  extractUpperLowerBones(model) {
    const upperKeywords = [
    "Spine", "Chest", "Neck", "Head", "HeadTop", // adiciona HeadTop_End
    "Shoulder", "Arm", "ForeArm", "Hand",
    "Thumb", "Index", "Middle", "Ring", "Pinky"
    ];
    const lowerKeywords = [
     "Hips", "UpLeg", "Leg", "Knee", "Foot",
    "Toe", "ToeBase", "Toe_End" // dedos do pé
    ];
