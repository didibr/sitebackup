...
creatematerial() {
  ...
  const grassTexture = textureLoader.load('./terrain/s4.jpg');
  const rockTexture = textureLoader.load('./terrain/s4.jpg');
  const snowTexture = textureLoader.load('./terrain/s4.jpg');
  ...
  material.onBeforeCompile = (shader) => {
      ...
      shader.uniforms.rockScale = { value: 0.08 };
  ...
  saveChunksToJSON() {
    ...
    localStorage.setItem('terrainData', json);
}
...
loadChunksFromJSON() {
  ...
  // 🔥 Reconstrói os chunks que já estão na cena
  ...
  getLocalModification(key, x, z) {
    ...
    for (const mod of data) {
      ...
      // 🔥 Aplica influência se estiver perto (raio de influência)
      if (dist < 5) { 
        const influence = (1 - dist / 5);
        totalDelta += mod.deltaHeight * influence;
      }
    }
    return totalDelta;
}
...
rebuildChunk(cx, cz) {
  ...
  for (let i = 0; i < vertices.count; i++) {
    ...
    let vy = this.generateHeight(vx, vz);

    const localMod = this.getLocalModification(key, vx, vz);
    if (localMod !== null) {
      vy += localMod;
    }
  ...
  createChunk(cx, cz) {
    ...
    for (let i = 0; i < vertices.count; i++) {
      ...
      // 🔥 Verifica modificações salvas
      const localMod = this.getLocalModification(key, vx, vz);
      if (localMod !== null) {
        vy += localMod; // aplica modificação
      }
  ...
  updateShadowCamera() {
    ...
    // Centraliza shadow camera na posição da câmera
