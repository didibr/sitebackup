...
if (indices.length > 0) {
  ...
  async setTPoseReference() {
    const gltf = await new Promise((resolve, reject) =>
      this.gltfloader.load(baseGLB, resolve, undefined, reject)
    );
    const model = gltf.scene || gltf.scenes[0]; // GLTFLoader retorna gltf, não um model direto
    model.updateMatrixWorld(true);
