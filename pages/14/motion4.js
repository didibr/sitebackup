...
retargetClipWickedStyle2(clip, sourceModel, targetModel) {
  const boneMap = {};
  sourceModel.updateMatrixWorld(true);
  targetModel.updateMatrixWorld(true);
  ...
  if (this.useHips || this.usePosition) {
    ...
    for (const track of posTracks) {
      ...
      // 🧩 Captura posição do primeiro frame para normalizar
      ...
      for (let i = 0; i < track.times.length; i++) {
        ...
        // Corrige offset por animação
        const relativePos = pos.clone().sub(baseOffset);
...
async correctAllHipsSmart2(model, angleThresholdDeg = 10, fps = 30) {
  const baseModel = this.baseTPoseModel;
  const baseMixer = window.bmixer;
  const baseActions = window.X;
...
async correctAllHipsSmart3(model, angleThresholdDeg = 10, fps = 30) {
  const baseModel = this.baseTPoseModel;
  const baseMixer = window.bmixer;
  const baseActions = window.X;
...
async correctAllHipsSmart(model, angleThresholdDeg = 10, fps = 30) {
  ...
  async load(input, animations = null) {
    ...
    //await this.applyHipsOffsetToAllClips(model);
