...
function resetCameraSmooth() {
    ...
    controls.enabled = false; // impede mexer durante o retorno
}

function activateCamera() {
    returningCamera = false;
    controls.enabled = true; // impede mexer durante o retorno
}

function returnCameraCheck(){
if (returningCamera) {
        const speed = 0.05; // menor = mais lento

        camera.position.lerp(initialCameraPos, speed);
        controls.target.lerp(initialTarget, speed);

        const posDone =
            camera.position.distanceTo(initialCameraPos) < 0.01;
...
async function createScene() {
    ...
    let remaining = JSON.parse(localStorage.getItem("remainingGirls") || "[]");
    // Se acabou a lista, recria e embaralha
    ...
    // Pega a próxima
    const randomGirl = remaining.pop();
    // Salva o estado para a próxima execução
    localStorage.setItem("remainingGirls", JSON.stringify(remaining));    

    await preloadAnimations(randomGirl, animationsList);
    loadMMD(randomGirl, 'idle1.vmd');
    //PLAYER PLAY
    if (firstTime) {
        firstTime = false;
        if (useAudioPlayer)
...
async function changeAnimation(nome, tempo = 0.5) {
    if (nome === currentAnimation) return true;
    ...
    const oldAction = animationMixer._actions.find(action => action._clip.name === currentAnimation);
    animationMixer._actions.forEach(action => {
    ...
    return new Promise(resolve => {
        blendResolve = resolve;
        const start = performance.now();
        function updateCh() {
            let t = (performance.now() - start) / (tempo * 1000);
            if (t > 1) t = 1;
            if (oldAction) oldAction.weight = 1 - t;
            newAction.weight = t;
            if (t < 1) {
...
async function PosicaoDefinida(nome, valor, callback) {
    function moverPara(target, range) {
        return new Promise(resolve => {
            let speed = 0.3;
    ...
    function rotacionarPara(target, range) {
        return new Promise(resolve => {
            clearInterval(temporizadorPOS);
            temporizadorPOS = setInterval(() => {
    ...
    if (CONFIG[nome]) {
        ...
        const iniciarMovimento = async () => {
            if (cfg.animStart) await changeAnimation(motionN(cfg.animStart));
            await moverPara(cfg.pos, cfg.range);
            if (cfg.animEnd) await changeAnimation(motionN(cfg.animEnd));
            if (cfg.next) {
                await PosicaoDefinida(cfg.next, "", callback);
            } else {
        ...
        if (cfg.rotateFirst && distancia >= 1) {
            await PosicaoDefinida(cfg.rotateFirst);
            await iniciarMovimento();
        } else {
            await iniciarMovimento();
        }
        return;
    ...
    if (nome == 'frente') {
        await rotacionarPara(0, 0.2);
        if (callback) callback();
        return;
    }
    if (nome == 'costas') {
        await rotacionarPara(3.2, 0.2);
        if (callback) callback();
        return;
    }
}
...
async function delayedinitial() {
    if (danceGOTOINIAL == null) {
        await PosicaoDefinida('inicial');
...
async function delayedintro() {
    if (danceSLOWINTRO == null) {
        await changeAnimation(motionN('intro'), 0.5);
