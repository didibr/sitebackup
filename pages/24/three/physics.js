...
class IPHYSICS {
    ...
    removeObj(oject) {
        if (typeof (oject) == this._UN || oject == null) return;
        if (typeof (oject.userData) == this._UN) return false;
        if (typeof (oject.userData.physicsBody) == this._UN) return false;
        oject.userData.physicsBody.setCollisionFlags(4);
        this.scene.remove(oject);
    ...
    bodyRotateQuaternion(object, quaternion) {
        ...
        if (typeof (quat.quaternion) != this._UN) quat = quat.quaternion;
        if (typeof (quat.x) == this._UN) { console.error("Invalid Quaternion"); return; }
        if (typeof (body) == this._UN || typeof (body) == this._UN) return;
    ...
    bodyRotateXYZ(object, xyz) {
        ...
        if (typeof (xyz.x) == this._UN) { console.error("Invalid Rotation"); return; }
        if (typeof (body) == this._UN || typeof (body) == this._UN) { console.error("Invalid Body"); return; }
        var obx = this.dummy;
    ...
    bodyTeleport(object, position) {
        var p = position;
        var body = object.userData.physicsBody;
        if (typeof (p) == this._UN || typeof (p.x) == this._UN) p = p.position;
        if (typeof (p.x) == this._UN) { console.error("Invalid Position"); return; }
        if (typeof (body) == this._UN || typeof (body) == this._UN) return;
        this.transformAux1.setIdentity();
    ...
    update(delta) {
        if (typeof (this.physicsWorld) == this._UN || this.physicsWorld == null || this.paused == true) return;
        ...
        for (let i = 0; i < numManifolds; i++) {
            ...
            for (let j = 0; j < numContacts; j++) {
                ...
                //só considera se realmente colidiu
                if (distance < 0) {
                    const impulse = contactPoint.getAppliedImpulse();
                    // ignora impactos fracos
                    if (impulse < 1) continue
                    //console.log(impulse);
                    // pega objetos THREE associados                    
                    const objA = body0.threeObject;
                    const objB = body1.threeObject;
                    ...
                    //CHAMA BREAKER (se existir)

                    if (this.breaker) {
                        if (this.breaker.isBreakable(objA)) {
                            this.breaker.collision(objA, objB, impulse, point, n);
                        }
                        if (this.breaker.isBreakable(objB)) {
                            this.breaker.collision(objB, objA, impulse, point, n);
                        }
                    }
                }
            }
    ...
    */
            ...
            case 'hull':
                ...
                obj.traverse(child => {
                    if (!child.isMesh || !child.geometry || child.isDebugPhysics)
                        return;
                    geom = child.geometry.index
                        ? child.geometry.toNonIndexed()
                        : child.geometry;
                    const vertices = geom.attributes.position.array;
                    for (let i = 0; i < vertices.length; i += 9) {
                        this._v1.setValue(
                            vertices[i] * objScale.x,
                            vertices[i + 1] * objScale.y,
                            vertices[i + 2] * objScale.z
                        );
                        this._v2.setValue(
                            vertices[i + 3] * objScale.x,
                            vertices[i + 4] * objScale.y,
                            vertices[i + 5] * objScale.z
                        );
                        this._v3.setValue(
                            vertices[i + 6] * objScale.x,
                            vertices[i + 7] * objScale.y,
                            vertices[i + 8] * objScale.z
                        );
                ...
                shape = new Ammo.btGImpactMeshShape(triangle_mesh);
                shape.updateBound();
                ...
                if (this.debug || extra?.debug) {
                    ...
                    obj.traverse(child => {
                        ...
                        geom = child.geometry.index
                            ? child.geometry.toNonIndexed()
                            : child.geometry;
                        vertices.push(...geom.attributes.position.array);
                    });
                    meshmodel.setAttribute(
                        'position',
    ...
    obj.traverse(child => {
        if (child.isMesh && child.geometry && !child.isDebugPhysics) {
            geom = child.geometry.index !== null ? child.geometry.toNonIndexed() : child.geometry;
            child.updateWorldMatrix(true, false);
            const matrix = new this.THREE.Matrix4().copy(obj.matrixWorld).invert().multiply(child.matrixWorld);
            ...
            verticesPos = Float32Concat(verticesPos, transformed);
        }
    });
    if (verticesPos.length === 0) {
        console.warn('No vertices found in object for triangle collider');
        break;
    }
    if (this.debug || extra?.debug) {
        meshmodel = new this.THREE.BufferGeometry();
    ...
    for (let i = 0; i < verticesPos.length; i += 9) {
        this._v1.setValue(verticesPos[i] * objScale.x, verticesPos[i + 1] * objScale.y, verticesPos[i + 2] * objScale.z);
        this._v2.setValue(verticesPos[i + 3] * objScale.x, verticesPos[i + 4] * objScale.y, verticesPos[i + 5] * objScale.z);
        this._v3.setValue(verticesPos[i + 6] * objScale.x, verticesPos[i + 7] * objScale.y, verticesPos[i + 8] * objScale.z);
    ...
    const centroidMesh = new this.THREE.Vector3();
    for (let i = 0; i < verticesPos.length; i += 3) {
        centroidMesh.x += verticesPos[i];
        centroidMesh.y += verticesPos[i + 1];
        centroidMesh.z += verticesPos[i + 2];
    }
    centroidMesh.divideScalar(verticesPos.length / 3);
    if (extra?.ignoreCentroid)
        centroidMesh.set(0, 0, 0);
    if (this.debug || extra?.debug) {
        const points = [];
        for (let i = 0; i < verticesPos.length; i += 3) {
            points.push(new this.THREE.Vector3(
                (verticesPos[i] - centroidMesh.x) ,
                (verticesPos[i + 1] - centroidMesh.y) ,
                (verticesPos[i + 2] - centroidMesh.z) 
            ));
        }
    ...
    getBodyVelocity(object) {
        ...
        return {
            linear: new THREE.Vector3(lv.x(), lv.y(), lv.z()),
            angular: new THREE.Vector3(av.x(), av.y(), av.z())
        };
    }

    setBodyVelocity(object, linear, angular) {
        const body = object.userData.physicsBody;
        if (!body) return;
    ...
    bodyImpulse(object, vec) {
        const body = object.userData.physicsBody;
        body.applyCentralImpulse(
            new Ammo.btVector3(vec.x, vec.y, vec.z)
        );
    }
