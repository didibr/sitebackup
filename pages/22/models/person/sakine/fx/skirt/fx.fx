...
//フィルライト色
float3 BackLight = float3(92,122,120)/255.0;
//バックライト色
float3 FillLight = float3(255,182,193)/255.0;
//空色
...
float LightParam = 1;
//自己発色抑制値
float EmmisiveParam = 1.0;

//シャドウマップサイズ
#define SHADOWMAP_SIZE 1024
//ソフトシャドウ用ぼかし率
float SoftShadowParam = 0.3;
...
float4 BufferShadow_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
    ...
    float3 Specular = pow( max(0,dot( HalfVector, normalize(normal) )), SpecularPower*50 ) * AmbientColor
    				 + pow( max(0,dot( HalfVector2, normalize(normal) )), SpecularPower*45 ) * FillLight*0.1
    				 + pow( max(0,dot( HalfVector3, normalize(normal) )), SpecularPower*45 ) * BackLight*0.1;
    ...
    // スペキュラ適用
    Color.rgb += Specular;
    ShadowColor.rgb += Specular*1;
    ShadowColor.rgb = Color.rgb*1;
